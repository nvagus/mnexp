...
class Cook:
    ...
    def get_doc_encoder(self):
        ...
        if self.config.news_encoder == 'cnnatt':
            title_d = keras.layers.Dropout(self.config.dropout)(title_e)
            title_c = keras.layers.Conv1D(*self.config.title_filter_shape, padding='same', activation='relu',
                                          strides=1)(
                title_d)
        ...
        elif self.config.news_encoder == 'cnn':
            title_d = keras.layers.Dropout(self.config.dropout)(title_e)
            title_c = keras.layers.Conv1D(*self.config.title_filter_shape, padding='same', activation='relu',
                                          strides=1)(
                title_d)
            title_m = keras.layers.Lambda(
                lambda x: x[0] * keras.backend.expand_dims(
                    keras.backend.switch(
                        ...
                        keras.backend.ones_like(x[1], dtype=keras.backend.floatx()))))([title_c, title])
            title_m = keras.layers.Masking()(title_m)
            title_a = models.GlobalAveragePoolingMaskSupport()(keras.layers.Dropout(self.config.dropout)(title_m))
        elif self.config.news_encoder == 'lstm':
            title_d = keras.layers.Dropout(self.config.dropout)(title_e)
            title_m = keras.layers.Lambda(
                lambda x: x[0] * keras.backend.expand_dims(
                    keras.backend.switch(
                        keras.backend.equal(x[1], 0),
                        keras.backend.zeros_like(x[1], dtype=keras.backend.floatx()),
                        keras.backend.ones_like(x[1], dtype=keras.backend.floatx()))))([title_d, title])
            title_m = keras.layers.Masking()(title_m)
            title_a = keras.layers.LSTM(self.config.title_filter_shape[0])(title_m)
        elif self.config.news_encoder == 'lstmatt':
            title_d = keras.layers.Dropout(self.config.dropout)(title_e)
            title_m = keras.layers.Lambda(
                lambda x: x[0] * keras.backend.expand_dims(
                    keras.backend.switch(
                        ...
                        keras.backend.ones_like(x[1], dtype=keras.backend.floatx()))))([title_d, title])
            title_m = keras.layers.Masking()(title_m)
            title_m = keras.layers.LSTM(self.config.title_filter_shape[0], return_sequences=True)(title_m)
            title_a = models.GlobalAveragePoolingMaskSupport()(keras.layers.Dropout(self.config.dropout)(title_m))
        else:
        ...
        if self.config.use_vertical:
            if self.config.use_vertical_type == 'v':
                dense = keras.layers.concatenate([title_a, vert_m])
            elif self.config.use_vertical_type == 's':
                dense = keras.layers.concatenate([title_a, subvert_m])
            else:
    ...
    def get_user_encoder(self):
        if self.config.use_vertical:
            if self.config.use_vertical_type == 'v':
                input_dim = self.config.title_filter_shape[
                                0] + self.config.vertical_embedding_dim
            elif self.config.use_vertical_type == 's':
                input_dim = self.config.title_filter_shape[
                                0] + self.config.subvertical_embedding_dim
            else:
                input_dim = self.config.title_filter_shape[
        ...
        elif self.config.arch == 'iavg':
            clicked_vec = models.GlobalAveragePoolingMaskSupport()(ch_m)
            user_vec = keras.layers.concatenate([clicked_vec, idx_vec])
        elif self.config.arch == 'iatt':
            clicked_vec = models.SimpleAttentionMaskSupport()(ch_m)
            user_vec = keras.layers.concatenate([clicked_vec, idx_vec])
        elif self.config.arch == 'ilstm':
            clicked_vec = keras.layers.LSTM(self.config.user_embedding_dim)(ch_m)
            user_vec = keras.layers.concatenate([clicked_vec, idx_vec])
    ...
    def _build_model(self):
        ...
        if self.config.use_vertical:
            if self.config.use_vertical_type == 'v':
                ch_vec = keras.layers.concatenate([ch_title_vec, ch_vert_vec])
            elif self.config.use_vertical_type == 's':
                ch_vec = keras.layers.concatenate([ch_title_vec, ch_subvert_vec])
            else:
...
class CookFT:
    ...
    def _build_model(self):
        ...
        if self.config.arch == 'dssm':
            ...
            f1 = keras.layers.Dense(1, activation='relu')(features_i)
            fd = keras.Sequential([
        ...
        self.train_model.compile('adam', self.loss, [self.acc])

    def loss(self, y_true, y_pred):
        y_true = tf.reshape(y_true, (-1, 1))
        return -tf.reduce_mean(
            self.config.gain * y_true * tf.log(y_pred + 1e-8) + (1 - y_true) * tf.log(
                1 - y_pred + 1e-8) / self.config.negative_samples)

    def acc(self, y_true, y_pred):
        y_true = tf.round(y_true)
        y_pred = tf.round(y_pred)
        return (self.config.negative_samples + 1) * 0.5 * tf.reduce_mean(
            y_true * y_pred + (1 - y_true) * (1 - y_pred) / self.config.negative_samples)
...
class CookBin(Cook):
    def get_doc_encoder(self):
        ...
        title_a = keras.layers.GlobalMaxPooling1D()(keras.layers.Dropout(self.config.dropout)(title_m))
    ...
    def get_user_encoder(self):
        ...
        idx_mask = keras.Input((1,))
        ch = keras.Input((self.training_data['ch_title'].shape[-2], input_dim))
        target = keras.Input((input_dim,))

        ch_m = keras.layers.Masking()(ch)

        if self.config.arch == 'cnn':
            user_vec = keras.layers.GlobalMaxPooling1D()(ch)
        elif self.config.arch == 'dkn':
            user_vec = models.QueryAttention()([ch_m, target])
        else:
            raise NotImplementedError

        return keras.Model([idx, idx_mask, ch, target], user_vec)
    ...
    def _build_model(self):
        ...
        cd_vecs = [doc_encoder([title, vert, subvert]) for title, vert, subvert in
                   zip(cd_titles, cd_verts, cd_subverts)]
        cd_vec_one = doc_encoder([cd_title_one, cd_vert_one, cd_subvert_one])
        user_vecs = [user_encoder([idx, idx_mask, ch_vec, cd_vec]) for cd_vec in cd_vecs]
        user_vec_one = user_encoder([idx, idx_mask, ch_vec, cd_vec_one])

        scorer = self.get_score_model(user_vec_one, cd_vec_one)

        score = keras.layers.Activation('sigmoid')(scorer([user_vec_one, cd_vec_one]))
        scores = [scorer([user_vec, cd_vec]) for user_vec, cd_vec in zip(user_vecs, cd_vecs)]
        logits = keras.layers.Activation('softmax')(keras.layers.concatenate(scores))
    ...
    def callback(self, epoch):
        if epoch in self.config.lrd_on_epochs:
            keras.backend.set_value(
                self.train_model.optimizer.lr,
                keras.backend.get_value(self.train_model.optimizer.lr) * self.config.lrd_on_epochs)
